...
- Header: `border-bottom: 3px solid #044B2E`, title full width (`max-width: none`). Logo placement depends on the title, see Logo below
...
- Legend: each item is `.legend > span { display: inline-flex; align-items: center; }`. Do NOT center the swatch with `vertical-align: middle` — that aligns to the text's x-height, not its optical center, and leaves every swatch sitting visibly low. Flex centering is the only thing that actually centers it.

Logo: `<img src="../../daily-journal-platform/public/images/logo/logo-green-horizontal.png">`. **Title fits on one line with the logo beside it → logo in the header**: `.header { display:flex; align-items:center; justify-content:space-between; gap:24px }`, `.header .logo { height:18px; flex-shrink:0 }` (right-aligned, vertically centered on the title). **Title would wrap with the logo there → logo goes inside the chart instead** (a header logo steals width and wraps long titles): an absolutely-positioned `.chart-logo` (height ~15px) inside the `position:relative` `.chart-container`. Default bottom-right; move it to whichever plot quadrant is empty (visual judgment after rendering — a climbing series leaves the bottom-right open; a falling/left-heavy one needs it elsewhere). Align it to the plot, don't eyeball: measure with browser-eval (`Chart.getChart('<id>').chartArea` + the container's clientWidth/Height), then for bottom-right set `right = containerW − chartArea.right` (logo's right edge meets the x-axis line's end) and `bottom = containerH − chartArea.bottom + ~12` (a gap above the axis line). Canonical asset lives in daily-journal-platform; never duplicate into ai-newsletter.
