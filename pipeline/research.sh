...
$PRE_RESEARCH"
  ...
  local cmd="cd $ROOT_DIR && $runner $outfile done-$name $CLAUDE_CMD --model $CLAUDE_MODEL --allowedTools Read,Write,Bash,mcp__exa__web_search_exa,mcp__exa__web_fetch_exa --settings $CLAUDE_BLOCK_DOMAINS_SETTINGS < $prompt_file"
...
$SEEDS_URLS" > "$prompt_file"
    ...
    local runner="$DIR/tools/run-agent.sh"
    local cmd="cd $ROOT_DIR && $runner $SEEDS_OUT done-seeds $CLAUDE_CMD --model $CLAUDE_MODEL --allowedTools Read,Write,Bash,mcp__exa__web_search_exa,mcp__exa__web_fetch_exa --settings $CLAUDE_BLOCK_DOMAINS_SETTINGS < $prompt_file"
