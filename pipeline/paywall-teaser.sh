...
PROMPT_FILE="$DAY_DIR/.prompt-paywall-teaser.md"
...
TMUX_WIN="paywall-$DATE"
tmux new-window -n "$TMUX_WIN" -d \
  "cd $ROOT_DIR && $DIR/tools/run-agent.sh $OUTPUT_META done-paywall-$DATE $CLAUDE_CMD --model $CLAUDE_MODEL --allowedTools Read,Write < $PROMPT_FILE"

echo "  Watch: tmux select-window -t $TMUX_WIN"
tmux wait-for "done-paywall-$DATE"
