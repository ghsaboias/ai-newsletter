...
Write today's newsletter article in Brazilian Portuguese using the research file as your sole source material. Do not search the web. Every fact and link must come from the research.

## Setup

1. Determine today's date.
2. Set `DAY_DIR` to `pipeline/output/ai/<today>/`.
3. Run `python3 pipeline/tools/slim_research.py <DAY_DIR>` to produce `research-slim.json`.
4. Read `<DAY_DIR>/research-slim.json` — this is your sole source material.
5. Save output to `<DAY_DIR>/pt.md`.
