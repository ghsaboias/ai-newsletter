...
Your output will be merged with other clusters (HW, World) into the final research file.
...
4. Read `<DAY_DIR>/techmeme.txt` for a pre-research scan of current headlines. Use this as a starting map.
5. Your output file is `pipeline/output/ai/<today>/research-ai.json`.
...
2. **Landscape scan**: Review the Techmeme scan, then run 5 Exa searches to fill gaps.
...
Write to `pipeline/output/ai/<today>/research-ai.json`. Format:
